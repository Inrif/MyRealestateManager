...
class RMMapFragment : RMBaseFragment(),
    OnMapReadyCallback,
    ...
    private var mRealEstatesWithPhotos: List<RMAndPhotos>? = null
    ...
    override fun onMarkerClick(marker: Marker?): Boolean {
        ...
        realEstateId?.let {
            // By action (Safe Args)
            val action = RMMapFragmentDirections.actionLocationFragmentToDetailsFragment(it)
    ...
    private fun configureSupportMapFragment() {
        ...
        childFragment?.getMapAsync(this@RMMapFragment)
    }
     ...
     */
    private fun addPointsOfInterest(realEstateWithPhotos: List<RMAndPhotos>) {
        // To avoid the NullPointerException during the rotation screen for example
        this.mGoogleMap?.let {
            // Remove Markers
            it.clear()
            ...
            realEstateWithPhotos.forEach { poi ->
                val location = LatLng(
                    poi.rm?.address?.latitude!!,
                    poi.rm?.address?.longitude!!
                )
